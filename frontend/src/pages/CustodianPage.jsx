        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '2rem' }}>
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', gap: '1rem' }}>
            <div style={{ ...card, padding: '0.75rem 1.25rem', marginBottom: 0, textAlign: 'center' }}>
              <div style={{ color: '#b45309', fontSize: '1.4rem', fontWeight: 800 }}>{pending.length}</div>
              <div style={{ color: '#64748b', fontSize: '0.75rem' }}>Pending</div>
            </div>
            <div style={{ ...card, padding: '0.75rem 1.25rem', marginBottom: 0, textAlign: 'center' }}>
              <div style={{ color: '#15803d', fontSize: '1.4rem', fontWeight: 800 }}>{bookings.filter(b => b.status === 'Approved').length}</div>
              <div style={{ color: '#64748b', fontSize: '0.75rem' }}>Approved</div>
            </div>
            <div style={{ ...card, padding: '0.75rem 1.25rem', marginBottom: 0, textAlign: 'center' }}>
              <div style={{ color: '#2563eb', fontSize: '1.4rem', fontWeight: 800 }}>{upcoming.length}</div>
              <div style={{ color: '#64748b', fontSize: '0.75rem' }}>Upcoming Slots</div>
            </div>
          </div>
        </div>
        {/* Tabs */}
            ...
            {tab === 'Mark Unavailable' && (
              <div style={card}>
                ...
                <form onSubmit={markUnavailable} style={{ display: 'flex', flexDirection: 'column', gap: '1rem', maxWidth: '480px' }}>
                  {[{ label: 'Hall', content: <select value={hallId} onChange={e => setHallId(e.target.value)} style={{ padding: '0.65rem', background: '#f8fafc', border: '1px solid #cbd5e1', borderRadius: '0.5rem', color: '#0f172a', fontSize: '0.95rem' }}><option value="" disabled>Select a hall</option>{halls.map(h => <option key={h._id} value={h._id}>{h.name}</option>)}</select> },
                    { label: 'Date', content: <input type="date" value={date} onChange={e => { setDate(e.target.value); setStartTime(''); setEndTime('') }} style={{ padding: '0.65rem', background: '#f8fafc', border: '1px solid #cbd5e1', borderRadius: '0.5rem', color: '#0f172a', fontSize: '0.95rem' }} /> }
                  ].map(f => <label key={f.label} style={{ color: '#475569', fontSize: '0.875rem', fontWeight: 600, display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>{f.label}{f.content}</label>)}
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                    <label style={{ color: '#475569', fontSize: '0.875rem', fontWeight: 600, display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>Start Time
                      <select value={startTime} onChange={e => { setStartTime(e.target.value); setEndTime('') }} style={{ padding: '0.65rem', background: '#f8fafc', border: '1px solid #cbd5e1', borderRadius: '0.5rem', color: '#0f172a', fontSize: '0.95rem' }}>
                        <option value="" disabled>Start</option>
                        {TIME_POINTS.slice(0, -1).map(t => <option key={t} value={t}>{t}</option>)}
                      </select>
                    </label>
                    <label style={{ color: '#475569', fontSize: '0.875rem', fontWeight: 600, display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>End Time
                      <select value={endTime} onChange={e => setEndTime(e.target.value)} disabled={!startTime} style={{ padding: '0.65rem', background: '#f8fafc', border: '1px solid #cbd5e1', borderRadius: '0.5rem', color: startTime ? '#0f172a' : '#94a3b8', fontSize: '0.95rem' }}>
